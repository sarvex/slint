...
pub fn print_rustc_flags() -> std::io::Result<()> {
        ...
        })
    {
        ...
        let mut toml = config.parse::<toml_edit::Document>().expect("invalid board config toml");
        // Fix up relative paths
        if let Some(link_search_path) =
            toml.get_mut("link_search_path").map(toml_edit::Item::as_array_mut).flatten()
        {
            for path_value in link_search_path.iter_mut().filter(|value| value.is_str()) {
                if path_value.as_str().unwrap() == "." {
                    *path_value =
                        board_config_path.parent().unwrap().to_string_lossy().as_ref().into();
                }
            }
        }
        ...
        for link_search_path in toml
            ...
            .flatten()
        {
            if let Some(option) = link_search_path.as_str() {
                println!("cargo:rustc-link-search={}", option);
            }
        }
    }
